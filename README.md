...
> **평가 조건:** 시드·bulk 샘플 DB 기준, hold-out 검증 (RF 20% / 랭킹 스크랩 hold-out 최대 80건).  
> 재현: `python scripts/evaluate_current_model.py` — 로컬 DB·학습 시점에 따라 소수 %p 차이 날 수 있습니다.
...
|---|---|
| 모델 버전 | v4 |
| ML 학습 샘플 | 1,765건 (게시글 1,454 + 스크랩 311) |
| 콘텐츠 인덱스 게시글 | 1,454건 |
| PostViews | 1,479건 |
| RF 피처 수 | 38 |

**1단계 — Random Forest (여행지·국가 예측)**

| 지표 | 의미 | 값 |
|---|---|---|
| **Top-1 Accuracy** | 예측 1순위 국가 적중 | **23.8%** |
| **Top-3 Accuracy** | 예측 3순위 내 국가 적중 | **49.6%** |

**2단계 — Stage3 하이브리드 랭킹 (For You 게시글 3건)**

| 지표 | 의미 | Stage3 | RF-only 랭킹 |
|---|---|---|---|
| **국가 Hit@3** | 추천 3건 중 실제 스크랩 국가 포함 | **32.5%** | 11.3% |
| **게시물 Hit@3** | 추천 3건 중 실제 스크랩 글 포함 | **8.7%** | — |

Stage3 국가 Hit@3는 RF-only 대비 **약 2.9배** (11.3% → 32.5%).  
LTR 학습 가중치(예): content **65%** · collab **34%** · view **1%**.

**지표 해석 (포트폴리오)**

- **국가 수준** 추천은 RF + Stage3 조합으로 hold-out에서 개선이 확인됨.
- **개별 게시물** Hit@3(8.7%)는 아직 낮음 — 콜드스타트·데이터 규모 한계.
- Precision@K가 아닌 **Top-k Accuracy / Hit@3** 기준 오프라인 평가임.
